#!/usr/bin/env bash
# harness: outputs=1
# Floating windows honor default_floating_size as usable-area fractions,
# per axis, and default_floating_size_px (pixels) outranks them. The default 1280x720 output
...
# TODO: Commenting this out for now; planned to be fixed in a later PR
# [[window_rule]]
...
default_floating = true
default_floating_size_px = [300, 200]
default_floating_size = [0.9, 0.9]
EOF
...
for title in frac-only frac-vs-pixels; do
...
echo "floating windows size from usable-area fractions per axis, and default_floating_size_px still wins"
